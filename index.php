...
// Normalize response shape from backend so template keys are predictable
$normalizedCategories = [];
$normalizedResult = [];
if (isset($data2['categories']) && is_array($data2['categories'])) {
	$normalizedCategories = $data2['categories'];
} elseif (isset($data2['data']) && is_array($data2['data'])) {
	$normalizedCategories = $data2['data'];
} elseif (isset($data2['result']) && is_array($data2['result'])) {
	foreach ($data2['result'] as $r) {
		$normalizedCategories[] = [
			'id' => $r['catId'] ?? ($r['id'] ?? ''),
			'category' => $r['category'] ?? ($r['name'] ?? '')
		];
	}
	$normalizedResult = $data2['result'];
}
$data2['categories'] = $normalizedCategories;
// expose 'data' for templates that use it
$data2['data'] = $normalizedCategories;
if (!isset($data2['result']) || !is_array($data2['result'])) {
	$data2['result'] = $normalizedResult;
}
