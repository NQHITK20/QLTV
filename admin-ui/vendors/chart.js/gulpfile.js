...
var argv = yargs
  ...
  .option("verbose", { default: false }).argv;
...
function buildTask() {
  var errorHandler = function (err) {
    if (argv.forceOutput) {
      ...
      ["Chart", "Chart.min", "Chart.bundle", "Chart.bundle.min"].forEach(
        function (fileName) {
...
function lintHtmlTask() {
  return gulp.src("samples/**/*.html").pipe(
    htmllint({
      failOnError: true,
    })
  );
...
function startTest() {
  return [
    ...
    "./src/**/*.js",
  ].concat(argv.inputs ? argv.inputs.split(";") : ["./test/specs/**/*.js"]);
}
...
function unittestTask(done) {
  new karma.Server(
    ...
    function (error) {
      error = error
        ? new Error("Karma returned with the error code: " + error)
        : undefined;
      done(error);
    }
  ).start();
...
function librarySizeTask() {
  return gulp.src("dist/Chart.bundle.min.js").pipe(
    size({
      gzip: true,
    })
  );
