										...
										<div class="tg-widgetcontent">
											<ul>
												...
												if (isset($data2['data']) && is_array($data2['data']) && count($data2['data'])>0) {
													foreach($data2['data'] as $cat) {
														$catName = htmlspecialchars((string)($cat['category'] ?? ''));
														$catHref = htmlspecialchars((string)($cat['category'] ?? '#'));
														$booksCount = htmlspecialchars((string)($cat['booksCount'] ?? $cat['books_count'] ?? '0'));
												?>
												<li><a href="<?php echo $catHref ?>"><span> <?php echo $catName ?></span><em><?php echo $booksCount ?></em></a></li>
												<?php }
												} else {
