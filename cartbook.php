								...
								<aside id="tg-sidebar" class="tg-sidebar">
									<div class="tg-widget tg-catagories">
										...
										<div class="tg-widgetcontent">
											<ul>
												<?php 
												if (isset($data2['data']) && is_array($data2['data']) && count($data2['data'])>0) {
													foreach($data2['data'] as $cat) {
														$catName = htmlspecialchars((string)($cat['category'] ?? ''), ENT_QUOTES, 'UTF-8');
														$booksCount = htmlspecialchars((string)($cat['booksCount'] ?? $cat['books_count'] ?? '0'));
												?>
												<li><a href="findingbook.php?tukhoa=<?php echo urlencode($cat['category'] ?? '') ?>" onclick="setCookie('tukhoa', '<?php echo $catName ?>', 30);"><span> <?php echo $catName ?></span><em><?php echo $booksCount ?></em></a></li>
												<?php }
												} else {
