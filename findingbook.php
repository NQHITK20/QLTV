...
// Lấy từ khóa tìm kiếm ưu tiên từ URL (?tukhoa=...), nếu không có thì fallback về cookie
$keyword = '';
if (isset($_GET['tukhoa']) && $_GET['tukhoa'] !== '') {
	$keyword = $_GET['tukhoa'];
	// Đồng bộ lại cookie để các trang khác vẫn dùng được nếu cần
	setcookie('tukhoa', $keyword, time() + 30 * 24 * 60 * 60, '/');
} elseif (!empty($_COOKIE['tukhoa'])) {
	$keyword = $_COOKIE['tukhoa'];
}

// Dữ liệu gửi đi
$datanew4 = array('tukhoa' => $keyword);
...
// ===== Yêu thích & giỏ hàng cho header (giống bookdetail) =====
...
if ($idusername) {
	...
	// Không có idusername: đảm bảo biến tồn tại để header không lỗi
								...
								<div id="tg-content" class="tg-content">
									<div class="tg-products">
										<div class="tg-sectionhead">
											<h2><?php echo htmlspecialchars($keyword, ENT_QUOTES, 'UTF-8'); ?></h2>
										</div>
